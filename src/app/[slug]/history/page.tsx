  ...
  return (
    <div style={{ background: 'var(--bg)', minHeight: '100vh' }}>
      <div className="page-header">
        <Link prefetch={false} href={`/${slug}/menu`} className="btn btn-ghost btn-sm" style={{ minWidth: 'auto' }}>← Menu</Link>
      ...
      <div style={{ maxWidth: '480px', margin: '0 auto', padding: '16px', paddingBottom: '80px' }}>
        {loading ? (
          ...
          <div style={{ textAlign: 'center', padding: '60px 0' }}>
            ...
            <Link prefetch={false} href={`/${slug}/menu`} className="btn btn-primary">Browse Menu</Link>
          </div>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            {orders.map(order => (
              <Link prefetch={false} key={order.id} href={`/${slug}/order-status/${order.ticket_number}`} style={{ textDecoration: 'none' }}>
